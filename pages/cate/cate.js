...
Page({
  ...
  //页面加载
  onLoad: function () {
    wx.setStorageSync('isAuthorized', app.globalData.accredit)
    //选取插件loading并显示
    ...
    /**
     * 在网络状态不好的情况下：在首页请求个人信息没有完成时，直接点击分类页，此时是没有openid
     * 所以此时需要重新先获取个人信息，再请求接口
    */
    if (!wx.getStorageSync('lushang_userInfo').openid) {
  ...
  checkResult(that) {
    if (this.data.banner_load && this.data.cate_load) {
  ...
  onShow() {
    /**
     * 在此页面授权后重新请求数据，相当于下拉刷新
    */
    var isAuthorized = wx.getStorageSync('isAuthorized')
    if (isAuthorized == false && app.globalData.accredit == true) {
  ...
  get_banner(again) {
    ...
    wx.request({
      ...
      data: {
        token: app.DP({ openid: openid, session_id: session_id, is_staff: 0 }),
        openid: openid,
